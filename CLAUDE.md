...
**第一關手機點選出牌模式（2026-09-04 應使用者要求補上，參考 `Rummikub` 的 `selectTile`/`placeToSet` 點選出牌模式）**：拖曳在手機上不好操作，因此在既有 Pointer Events 拖曳邏輯之外，額外疊加一套「點一下卡片選取→點一下目標格放置」的替代互動，兩者並存、互不衝突，不是切換模式。關鍵是**用位移量分流同一組 pointerdown/pointerup 事件**：`pointerdown` 先只記錄到 `pending`（不立即進入拖曳視覺狀態），`pointermove` 累積位移超過 `TAP_MOVE_THRESHOLD`（6px）才呼叫 `promoteToDrag()` 升級成真正的 `drag` 狀態（沿用原本 fixed 定位拖曳邏輯）；若 `pointerup` 發生時仍停留在 `pending`（沒超過位移門檻），視為「點擊」呼叫 `handleCardTap()`，改走選取／放置邏輯，不會誤觸拖放。`selectedCard` 只允許同時選一張（`selectCard()`/`clearSelection()`），卡片與流程圖 slot 皆加上 `click` 監聽（slot 的監聽器用 `e.target.closest('.card')` 判斷該次點擊是否落在卡片本身，是的話交給 `handleCardTap()` 處理、避免重複觸發），手牌區 `handEl` 也有 `click` 監聽讓已選取的卡片可以點擊空白處送回手牌。`placeSelectedIntoSlot()` 與既有拖放的佔用互換邏輯一致（目標格已有卡片時，原卡片自動送回手牌）。新增 `.card.selected`（琥珀色外框＋上浮）與 `.flow-board.has-selection .slot` 脈動提示樣式協助辨識目前選取與可放置目標。`setupLevel1()`／判分鎖定時都會呼叫 `clearSelection()` 清空殘留選取狀態。已用 Playwright 派發 PointerEvent（含刻意不超過位移門檻的點擊、以及超過門檻的真實拖曳）在單一 evaluate 內驗證：選取→放置、放置到已佔用格觸發交換、選取已放置卡片→點手牌區送回、真實拖曳仍正常運作，四種情境皆通過。
